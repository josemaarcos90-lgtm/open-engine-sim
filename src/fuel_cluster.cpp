...
void FuelCluster::render() {
    ...
    // A freshly rebuilt UI can render in the same tick before its first
    // update(). Never assume the simulator has already been propagated.
    const Vehicle *vehicle = (m_simulator != nullptr) ? m_simulator->getVehicle() : nullptr;
    const double travelledDistance = (vehicle != nullptr)
